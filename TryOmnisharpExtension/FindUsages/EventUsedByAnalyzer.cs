...
[Export]
public class EventUsedByAnalyzer
{
    private readonly AnalyzerScope _analyzerScope;
    const GetMemberOptions Options = GetMemberOptions.IgnoreInheritedMembers | GetMemberOptions.ReturnMemberDefinitions;

    [ImportingConstructor]
    public EventUsedByAnalyzer(AnalyzerScope analyzerScope)
    {
        _analyzerScope = analyzerScope;
    }

    public async Task<IEnumerable<ISymbol>> Analyze(IEvent analyzedSymbol)
    {
        var analyzedMethod = (IEvent)analyzedSymbol;
        ...
        foreach (var type in typesInScope)
        {
            ...
            {
                if (IsUsedInMethod((IEvent)analyzedSymbol, method))
                {
            ...
            {
                if (property.CanGet && IsUsedInMethod((IEvent)analyzedSymbol, property.Getter))
                {
                    result.Add(property);
                    continue;
                }

                if (property.CanSet && IsUsedInMethod((IEvent)analyzedSymbol, property.Setter))
                {
                    result.Add(property);
                    continue;
                }
            ...
            {
                if (@event.CanAdd && IsUsedInMethod((IEvent)analyzedSymbol, @event.AddAccessor))
                {
                    result.Add(@event);
                    continue;
                }

                if (@event.CanRemove && IsUsedInMethod((IEvent)analyzedSymbol, @event.RemoveAccessor))
                {
                    result.Add(@event);
                    continue;
                }

                if (@event.CanInvoke && IsUsedInMethod((IEvent)analyzedSymbol, @event.InvokeAccessor))
                {
                    result.Add(@event);
                    continue;
                }
    ...
    bool IsUsedInMethod(IEvent analyzedEntity, IMethod method)
    {
        return ScanMethodBody(analyzedEntity, method, method.GetMethodBody());
    }

    static bool ScanMethodBody(IEvent analyzedMethod, IMethod method, MethodBodyBlock methodBody)
    {
        if (methodBody == null)
            return false;
