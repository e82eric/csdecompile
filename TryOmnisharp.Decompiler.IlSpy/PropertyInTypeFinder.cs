...
{
    public (UsageAsTextLocation, string) Find(IProperty symbol, EntityHandle? setterEntityHandle, EntityHandle? getterEntityHandle, ITypeDefinition rootType)
    {
        var fileName = symbol.ParentModule.PEFile.FileName;
        var cachingDecompiler = DecompilerFactory.Get(fileName);

        var (syntaxTree, sourceText) = cachingDecompiler.Run2(rootType);
