...
public class CachingDecompiler
{
    ...
    public CachingDecompiler(string fileName)
    {
        ...
        _typeSystem = typeSystemFactory.GetOrCreateTypeSystem(findAssembly.GetPEFileOrNull());
    }

    public ITypeDefinition FindTypeDefinition(string fullTypeName)
    {
        var findType = _typeSystem.FindType(new FullTypeName(fullTypeName)) as ITypeDefinition;
    ...
    public Tuple<SyntaxTree, string> Run2(ITypeDefinition rooTypeDefinition)
    {
        ...
        var decompiler = new CSharpDecompiler(_typeSystem, decompilerSettings);
