...
{
    public (UsageAsTextLocation, string) Find(ITypeDefinition symbol, EntityHandle handleToSearchFor, ITypeDefinition rootTypeHandle)
    {
        var fileName = symbol.ParentModule.PEFile.FileName;
        var cachingDecompiler = DecompilerFactory.Get(fileName);
        var (syntaxTree, sourceText) = cachingDecompiler.Run2(rootTypeHandle);
